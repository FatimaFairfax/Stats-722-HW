...
We can find this using pbeta and the posterior values. **coming back to realize I need to flip this because the formula calculate the pi < value**.
...
This is a ration of the prior probability of H_a and H_0
...
Ah! This formula is giving the probability that pi is < 0.1 and I need pi > 0.1. So I can take this value minus 1. 
...
This is the probability that the new observation (Y') will be at least 20 given the observed data gave Y=150. This can be found by a long formula...

$f(y'|y=150) = {100}\choose{y'} * \frac{\Gamma(1003)}{\Gamma(151)*\Gamma(852)} * \frac{\Gamma(151 + y')*\Gamma()}...
...
This gives that the average of this sample is about 201 and the number of species data points is 344
...
First to calculate a posterior for mu

```{r}
summarize_normal_normal(200,40,y_bar=200.9152,n=344)
```


This gives a posterior of N(200,40^2)...which is the same...hmmm

To get a 95% credible interval we can use qnorm

```{r}
qnorm(c(0.025,0.975),mean=200,sd=40)
```

This gives a 95% CI of (121.6,278.4). This means that there is a 95% posterior probability that the mean is between 121.6 and 278.4.
...
I will use s = 2 and r = 100 to keep the units consistent with the units we observed
...
Firs to use the summary to find the posterior model:
```{r}
summarize_gamma_poisson(2,100,sum_y=1.5,n=18)
```

This gives a posterior Gamma(3.5,118)

Then using this, we can calculate the 95% CI using qgamma
```{r}
qgamma(c(0.025,0.975),3.5,118)
```

*The 95% CI is (0.0072, 0.0679). This means that there is a 95% posterior probability that lambda is between 0.007 and 0.07.*
...
The CI we got for lambda were both below 1 by a bit, so we would expect to favor heavily the null hypo
...
Again, we have to flip this probability because this is giving the probability that lambda < 1 as the formula takes it. Meaning that the posterior probability of the alternative hypothesis that lambda >=1 is 0. Let's check out the graph to see if that checks out

```{r}
plot_gamma(3.5,118)
```

This seems to check out, providing all lambda values under 0.1
...
```
This provides a 95% CI of (0.164, 0.342)
...
This gives a 95% CI of (0.164,0.342) as well
...
This says that the posterior probability that lambda is less than 1 is 1, which is what we found before

*e. How close are the approximations to the actual posterior values you calculated?*

Actual calculations:

Posterior probability of lambda < 1 = 1
95% CI = (0.0072, 0.0679)
posterior mean = .0297
...
loons_chain_df <- loons_chain_df |> 
  mutate(y_predict=rpois(n=1,lambda))
...
This gives the most likely value to be 0. 
...
This provides a mean of 0 and a 95% chance that the number of loons the birdwatcher will see in the next 100-hour period is 0
