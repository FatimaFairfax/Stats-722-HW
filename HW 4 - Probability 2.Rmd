...
Most online examples I have seen are specifically about medical diagnosis but seems to be consistent with the B&H chapter.
...
while(probs_fair > 0.1) {
  probs_fair <-  fair_coin^head_count * 1/2 / (fair_coin^3 * 1/2 + biased_coin^head_count * 1/2)
...
This while loop tells us we need 5 heads in a row before the probability that we are using the fair coin dips below 10%.
...
This shows that we would need 7 heads in a row for the probability that we have the fair coin to dip below 5%.
...
fair_coin^tails * 1/2 / (fair_coin^3 * 1/2 + T_biased_coin^tails * 1/2)
```
