...
First step is that I will build each of the die:
```{r}
die_1 <- tibble(
  value = c(1,
      2,
...
Now I have the 3 weighted die. No I will look at the average for die_1 from a sample of 1000:
```{r}
...
So now I will create a die values vector and 3 probability vectors that correspond to the weights assigned:
```{r}
...
**I later learned how to add a line to show the average on othe chart! I will now go back and add that as well as the average output for each of these graphs**
